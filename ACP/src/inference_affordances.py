...
def infer(out_dir, model_ROI, model_YCB, n_classes, mask_location, frames, args):
	...
	for item in tqdm(images):
		...
		img, bb_list, vm_list, inp_ims = get_data_for_image_path(item, GT_item, args.imsize, transform, mask_location, wsize=args.wsize, hand_cond=args.hand_cond, fix_target=True)
		...
		# cluster_list = [str(i) for i in range(n_classes)]

		# if cluster_list is not None:
		# 	os.makedirs(f'{out_dir}/{item_name}', exist_ok=True)
		# 	for ind_ac, ac in enumerate(cluster_list):
		# 		final_npimage = np.where(npimg[:, :, 0] == 0, 0, final_npimage_[:, :, ind_ac].copy()) # black regions get 0 score
		# 		final_npimage = final_npimage #/ (np.max(final_npimage) + 1e-5)
...
def main(args):
	...
	infer(out_dir, model_ROI, model_YCB, n_classes, mask_location, frames, args)
