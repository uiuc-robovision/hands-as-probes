...
from data.Base_seg import EPICPatchLoaderSeg
...
class EPICPatchLoaderSegwHands(EPICPatchLoaderSeg):
    ...
    def __init__(self, config,
        ...
        super(EPICPatchLoaderSegwHands, self).__init__(
                config,
                split=split,
                transform=transform,
                length=length
    ...
    def __getitem__(self, item):
        ...
        # os.makedirs("temp_seg_hands", exist_ok=True)
        # inp_img.save(f"temp_seg_hands/inp_{item}.png")
        # seg_image.save(f"temp_seg_hands/out_im_{item}.png")
        # seg_mask.save(f"temp_seg_hands/out_mask_{item}.png")
...
if __name__ == "__main__":
    ...
    print(config)
    data = EPICPatchLoaderSegwHands(split="train",
                                        transform=True,
                                        config=config,
                                        )
    torch.manual_seed(0)
