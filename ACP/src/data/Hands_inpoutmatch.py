...
from data.Base_inpoutmatch import EPICPatchLoaderInpOutMatch
...
class EPICPatchLoaderInpOutMatchwHands(EPICPatchLoaderInpOutMatch):
    ...
    def __init__(self, config,
        ...
        super(EPICPatchLoaderInpOutMatchwHands, self).__init__(
                config,
                split=split,
                transform=transform,
                length=length
    ...
    def __getitem__(self, item):
        ...
        # os.makedirs("temp_hands_inpoutmatch", exist_ok=True)
        # inp_img.save(f"temp_hands_inpoutmatch/inp_{item}.png")
        # seg_image.save(f"temp_hands_inpoutmatch/out_im_{item}.png")
        # seg_mask.save(f"temp_hands_inpoutmatch/out_mask_{item}.png")
...
if __name__ == "__main__":
    ...
    config = load_config("../configs/base_hands_symmetric.yaml")
    print(config)
    data = EPICPatchLoaderInpOutMatchwHands(split="validation",
                                        transform=True,
                                        config=config,
                                        )
    torch.manual_seed(0)
