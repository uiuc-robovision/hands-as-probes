...
class Meter:
    ...
    def check(self, highest=False):
        avg = self.average()
        print(f"Avg {avg:.3f} Highest {self.highest:.3f} Lowest {self.lowest:.3f}")
        if highest:
            if avg > self.highest:
                self.highest = avg
                return 1
...
class APMeterBin:
    ...
    def value_random(self, size=100000):
        if len(self.labels) > 0:
            ind1 = self.rng.choice(np.arange(len(self.labels)), size=min(20, len(self.labels)), replace=False)
            labs = np.concatenate([self.labels[i] for i in ind1], axis=0).astype(np.int32)
            probs = np.concatenate([self.probs[i] for i in ind1], axis=0)
